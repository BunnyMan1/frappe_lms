<template>
		...
		<div class="flex justify-between w-full">
		  <div class="md:w-2/3">
			<div class="flex items-center space-x-2">
			  ...
			  <Badge :variant="'solid'" theme="gray" size="sm" class="p-3">
				{{ course.data.lms_course_category }}
			  </Badge>
			</div>
			...
			<div class="flex items-center">
			  <Tooltip v-if="course.data.avg_rating" :text="__('Average Rating')" class="flex items-center">
			...
			<div class="flex mt-3 mb-4 w-fit">
			  <Badge theme="gray" size="lg" class="mr-2" v-for="tag in course.data.tags">
				{{ tag }}
			  </Badge>
			</div>
  ...
  const parsedCourseParams = computed(() => JSON.parse(props.courseParams))
  ...
  const breadcrumbs = computed(() => {
	...
	items.push({
	  ...
	  route: { name: 'CourseDetail', params: { courseName: course?.data?.name }, query: { courseParams: props.courseParams } },
	})
	return items
  })
